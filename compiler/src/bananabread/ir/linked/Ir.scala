...
case class Id(expr: ast.Id, source: Option[ast.Import]) extends Ir with Print(s"(id ${source.map(_.name.id).getOrElse("self")}.${expr.lexeme})")
...
def lift(nodes: List[typeless.Ir], imports: List[ast.Import]): Lifted[List[Ir]] =
  nodes.foldLeft[Scoped[List[Ir]]](Right((List.empty, List.empty))) {
    case (Left(err), _) =>
      Left(err)
    case (Right((irs, locals)), node) =>
      lift(node, imports, locals).map { (lifted, newLocals) =>
        (irs :+ lifted, newLocals)
      }
  }.map(_._1)
def lift(node: typeless.Ir, imports: List[ast.Import], locals: List[ast.Id]): Scoped[Ir] = node match
  ...
  case node: typeless.Id     => liftId(node, imports, locals)
  case node: typeless.App    => liftApp(node, imports, locals)
  case node: typeless.Lambda => liftLambda(node, imports, locals)
  case node: typeless.Cond   => liftCond(node, imports, locals)
  case node: typeless.Let    => liftLet(node, imports, locals)
  case node: typeless.Begin  => liftBegin(node, imports, locals)
  case node: typeless.Def    => liftDef(node, imports, locals)

def liftId(node: typeless.Id, imports: List[ast.Import], locals: List[ast.Id]): Scoped[Id] =
  ...
  (local, source) match
    case (true, _) => Right((Id(node.expr, None), locals))
    case (_, source @ Some(_)) => Right((Id(node.expr, source), locals))
    case (_, _) => Left(UndeclaredIdentifierErr(node))

def liftApp(node: typeless.App, imports: List[ast.Import], locals: List[ast.Id]): Scoped[App] =
  for
    liftedLamda <- lift(node.lambda, imports, locals).map(_._1)
    liftedArgs  <- node.args.map(lift(_, imports, locals)).squished.map(_.map(_._1))
  yield
    (App(liftedLamda, liftedArgs, node.expr), locals)

def liftLambda(node: typeless.Lambda, imports: List[ast.Import], locals: List[ast.Id]): Scoped[Lambda] =
  for
    liftedBody <- lift(node.body, imports, locals ++ node.params.map(_.name)).map(_._1)
  yield
    (Lambda(node.params, liftedBody, node.tyVars, node.expr), locals)

def liftCond(node: typeless.Cond, imports: List[ast.Import], locals: List[ast.Id]): Scoped[Cond] =
  for
    liftedCond <- lift(node.cond, imports, locals).map(_._1)
    liftedPass <- lift(node.pass, imports, locals).map(_._1)
    liftedFail <- lift(node.fail, imports, locals).map(_._1)
  yield
    (Cond(liftedCond, liftedPass, liftedFail, node.expr), locals)

def liftLet(node: typeless.Let, imports: List[ast.Import], locals: List[ast.Id]): Scoped[Let] =
  for
    bindingsLift <- liftLetBindings(node, imports, locals)
    liftedBindings = bindingsLift._1._1
    recLocals = bindingsLift._1._2
    liftedBody <- lift(node.body, imports, recLocals).map(_._1)
  yield
    (Let(liftedBindings, liftedBody, node.expr), locals)

def liftLetBindings(node: typeless.Let, imports: List[ast.Import], locals: List[ast.Id]): Scoped[(List[Binding], List[ast.Id])] =
  node.bindings.foldLeft[Scoped[(List[Binding], List[ast.Id])]](Right(((List.empty, locals), locals))) {
    ...
    case (Right(((liftedBindings, locals), _)), binding) =>
      ...
      lift(binding.value, imports, recLocals).map(_._1).map { ir =>
        ((liftedBindings :+ Binding(binding.label, ir, binding.expr), recLocals), locals)
      }
  }

def liftBegin(node: typeless.Begin, imports: List[ast.Import], locals: List[ast.Id]): Scoped[Begin] =
  for
    liftedIns <- node.ins.map(lift(_, imports, locals)).squished.map(_.map(_._1))
  yield
    (Begin(liftedIns, node.expr), locals)

def liftDef(node: typeless.Def, imports: List[ast.Import], locals: List[ast.Id]): Scoped[Def] =
  for
    liftedValue <- lift(node.value, imports, locals :+ node.name).map(_._1)
  yield
