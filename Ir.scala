...
object Typeless:
  sealed trait Base
  case class Num(num: ast.Num) extends Base with Print(s"(num ${num.lexeme})")
  case class Str(str: ast.Str) extends Base with Print(s"(str ${str.lexeme})")
  case class Id(id: ast.Id) extends Base with Print(s"(id ${id.lexeme})")
  case class App(lambda: Base, args: List[Base], expr: Expr) extends Base with Print(s"(app lambda: ${lambda} args: (${args.mkString(" ")}))")
  case class Lambda(params: List[Base], body: Base, expr: Expr) extends Base with Print(s"(lambda params: (${params.mkString(" ")}) body: $body)")
  case class Cond(cond: Base, pass: Base, fail: Base, expr: Expr) extends Base with Print(s"(if cond: $cond then: $pass else: $fail)")
  case class Let(bindings: List[Binding], body: Base, expr: Expr) extends Base with Print(s"(let bindings: (${bindings.mkString(" ")}) body: $body)")

  case class Binding(label: ast.Id, value: Base, expr: ast.Binding) extends Print(s"binding: $label value: $value")

  def lift(exprs: List[Expr]): List[Base] =
    exprs.map(lift)

  def lift(expr: Expr): Base = expr match
